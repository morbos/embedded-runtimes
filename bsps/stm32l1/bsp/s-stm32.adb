...
package body System.STM32 is
   ...
   begin
      case Source is
         ...
         when SYSCLK_SRC_PLL =>
            declare
               Pllm   : constant Word := Word (RCC_Periph.CFGR.PLLMUL) + 1;
               Plln   : constant Word := Word (RCC_Periph.CFGR.PLLDIV);
--  Pllp   : constant Word := Word (RCC_Periph.PLLCFGR.PLLP);
               Pllp : constant Word := 1; --  !!!!!
               Pllvco : Word;

            begin
               if RCC_Periph.CFGR.PLLSRC then
                  Pllvco := (Param.HSE_Clock / Pllm) * Plln;
               else
                  Pllvco := (Param.HSI_Clock / Pllm) * Plln;
               end if;
               Result.SYSCLK := Pllvco / ((Pllp + 1) * 1);  --  !!!! was * 2
            end;
      end case;
