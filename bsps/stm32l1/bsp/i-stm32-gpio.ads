...
package Interfaces.STM32.GPIO is
   ...
   --  OSPEEDER_OSPEEDR array element
   subtype OSPEEDER_OSPEEDR_Element is Interfaces.Bit_Types.UInt2;

   --  OSPEEDER_OSPEEDR array
   type OSPEEDER_OSPEEDR_Field_Array is array (0 .. 15)
     of OSPEEDER_OSPEEDR_Element
     with Component_Size => 2, Size => 32;

   --  GPIO port output speed register
   type OSPEEDER_Register
     (As_Array : Boolean := False)
   is record
      case As_Array is
         when False =>
         ...
         when True =>
            --  OSPEEDR as an array
            Arr : OSPEEDER_OSPEEDR_Field_Array;
      end case;
   end record
     with Unchecked_Union, Size => 32, Volatile_Full_Access,
          Bit_Order => System.Low_Order_First;

   for OSPEEDER_Register use record
      Val at 0 range 0 .. 31;
      Arr at 0 range 0 .. 31;
   end record;
   ...
   type GPIO_Peripheral is record
      ...
      --  GPIO port output speed register
      OSPEEDER : aliased OSPEEDER_Register;
      --  GPIO port pull-up/pull-down register
